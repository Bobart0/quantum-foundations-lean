...
/-!
**FR.** # W5 — Assemblage (Bargmann §5) et théorème de Wigner
...
**EN.** # W5 — Assembly (Bargmann §5) and Wigner's theorem

U extends chi/V to the whole space through the decomposition
a = ⟪e,a⟫•e + (a −
⟪e,a⟫•e), whose second term lies in 𝒫. Compatibility
∀x, ∃c, T x = c•U x splits into two cases:
⟪e,x⟫ ≠ 0 (direct calculation) and ⟪e,x⟫ = 0 (FREE, through the
definitional collinearity in V_colinear; no Cauchy–Schwarz argument is
needed, contrary to the initial concern). Bijectivity of U (isometry ⟹ injective ⟹ surjective in finite
 dimension) is proved by restricting to the underlying ℝ-linear structure in
 the antiunitary branch. LinearMap.injective_iff_surjective is available only for
K-linear endomorphisms, whereas a conjugate-semilinear map over ℂ is ℝ-linear after restriction
of scalars. No coordinates are required.

Final statement: formulation (A), with no bijectivity hypothesis
(Bargmann §1.2–§1.3). Injectivity at the ray level follows from hT through
Cauchy–Schwarz, and in finite dimension the constructed U is automatically
bijective. This statement is strictly stronger than that of Simon et al.,
who assume bijectivity in Eq. 2.8.
-/
...
/--
**FR.** Compatibilité de `U` avec `T` à une phase près, sur la sphère unité. Cœur de
la preuve de `wigner`.

**EN.** Compatibility of U with T up to a phase, on the unit sphere. This is
the core of the proof of wigner.
-/
...
/--
**FR.** `U` préserve la norme, INDÉPENDAMMENT de la branche de `chi` (`inner_U_eq_chi_inner`
donne `⟪Ua,Ua⟫ = chi⟪a,a⟫`, et `chi` fixe le réel non-négatif `‖a‖²`).

**EN.** U preserves norms INDEPENDENTLY of the branch of chi:
inner_U_eq_chi_inner gives ⟪Ua,Ua⟫ = chi⟪a,a⟫, and chi fixes the
...
/--
**FR.** Cas `n = 1` : `𝒫` est réduit à `{0}` (`dim H 1 = 1`), donc tout `x` est un
multiple scalaire de `e 1` — fait autonome, indépendant de `hn : 2 ≤ n`.

**EN.** Case n = 1: 𝒫 reduces to {0} (dim H 1 = 1), so every x is a
scalar multiple of e 1. This is a self-contained fact, independent of
hn : 2 ≤ n.
-/
...
/--
**FR.** **Théorème de Wigner** (dimension finie, sans hypothèse de bijectivité sur
...
**EN.** Wigner's theorem (finite-dimensional, with no bijectivity hypothesis
on T). Every transformation of pure states that preserves transition
probabilities |⟨φ|ψ⟩|² is induced by a unitary or an antiunitary operator.
-/
