...
/-!
**FR.** # W6 (optionnel) — Unicité et exclusivité (Bargmann §1.5, §6 restreint)
...
**EN.** # W6 (optional) — Uniqueness and exclusivity (Bargmann §1.5, restricted §6)

Two independent, low-priority deliverables:
(A) Exclusivity of the unitary and antiunitary alternatives
(Bargmann §1.5, witness Delta = i/6).
(B) Uniqueness of U up to a global phase relative to the choice of
representative eImg (a RESTRICTED version, not the full generality of
Bargmann's Theorem 2, which would require rederiving real homogeneity from
additivity + isometry for a completely arbitrary U'. This is unnecessary
here, since all freedom in wigner arises from the choice of eImg).
-/
...
/-!
**FR.** ## (A) Exclusivité unitaire/antiunitaire (Bargmann §1.5)

**EN.** ## (A) Exclusivity of the unitary and antiunitary alternatives
(Bargmann §1.5)
-/

/--
**FR.** Produit cyclique de rayons, témoin de non-réalité (Bargmann §1.5).

**EN.** Cyclic product of rays, serving as a witness of nonreality
(Bargmann §1.5).
-/
...
/--
**FR.** Analogue semilinéaire de `LinearIsometryEquiv.inner_map_map`, absent de
Mathlib pour `σ ≠ id` — dérivé à la main par polarisation.

**EN.** Semilinear analogue of LinearIsometryEquiv.inner_map_map, absent from
Mathlib for σ ≠ id; derived directly by polarization.
-/
...
/--
**FR.** Étape 1, branche linéaire (`chi = id`) : `Delta` est invariant sous `T`.

**EN.** Step 1, linear branch (chi = id): Delta is invariant under T.
-/
...
/--
**FR.** Étape 1, branche antiunitaire (`chi = conj`) : `Delta` est envoyé sur son
conjugué sous `T`.

**EN.** Step 1, antiunitary branch (chi = conj): Delta is mapped to its
complex conjugate under T.
-/
...
/--
**FR.** Témoins de non-réalité de Bargmann §1.5 : deux vecteurs unitaires formant,
avec `e`, un triplet dont `Delta` n'est pas réel.

**EN.** Bargmann's witnesses of nonreality from §1.5: two unit vectors that,
together with e, form a triple whose Delta is not real.
-/
...
/--
**FR.** Étape 2 : calcul explicite `Delta(e, e2, e3) = i/6` (Bargmann §1.5).

**EN.** Step 2: explicit calculation Delta(e, e2, e3) = i/6
(Bargmann §1.5).
-/
...
/--
**FR.** Étape 3 : un même `T` ne peut être compatible simultanément avec une
équivalence unitaire ET une équivalence antiunitaire (Bargmann §1.5).

**EN.** Step 3: the same T cannot be simultaneously compatible with a unitary
equivalence AND an antiunitary equivalence (Bargmann §1.5).
-/
...
/-!
**FR.** ## (B) Unicité de `U` à phase globale près (version restreinte)
...
**EN.** ## (B) Uniqueness of U up to a global phase (restricted version)

eImg T := T (e n) is a FIXED def in Defs.lean, with no parameter for
choosing another unit representative of the same class. To state uniqueness,
this file introduces local parameterized versions
Vp/chidirp/chip/Up, obtained by replacing eImg T in the formulas of
Defs.lean with an explicit representative eImg0. Defs.lean is unchanged:
the lemmas V_eq_Vp/chi_eq_chip/U_eq_Up below show by rfl that these
parameterized versions, evaluated at eImg T, reproduce exactly
V/chi/U.

Scope: this is strictly weaker than Bargmann's Theorem 2 in §6, which would
cover a completely arbitrary U'. Only the freedom actually used by
wigner, namely the choice of representative of eImg, is treated.
-/

/--
**FR.** Version paramétrée localement de `V` : `eImg` explicite plutôt que fixé à
`eImg T := T (e n)`.

**EN.** Locally parameterized version of V: an explicit eImg rather than
the fixed value eImg T := T (e n).
-/
...
/--
**FR.** Pont vers `Defs.lean` : `Vp`/`chidirp`/`chip`/`Up` évalués au représentant
canonique `eImg T` redonnent exactement `V`/`chidir`/`chi`/`U`.

**EN.** Bridge to Defs.lean: Vp/chidirp/chip/Up, evaluated at the
canonical representative eImg T, reproduce exactly
V/chidir/chi/U.
-/
...
/--
**FR.** Étapes (i)+(ii) : `V` calculé au représentant `λ • eImg` vaut `λ • V` calculé
au représentant `eImg`, pour tout `‖λ‖ = 1`.

**EN.** Steps (i)+(ii): V computed with representative λ • eImg equals
λ • V computed with representative eImg, for every ‖λ‖ = 1.
-/
...
/--
**FR.** Étape (iii) : `χ` ne dépend PAS du choix du représentant `eImg` -- pas
seulement sa branche `id`/`conj`, la fonction entière.

**EN.** Step (iii): χ does NOT depend on the choice of representative
eImg—not merely its id/conj branch, but the entire function.
-/
...
/--
**FR.** Étape (iv) : `U` calculé au représentant `λ • eImg` vaut `λ • U` calculé au
représentant `eImg`.

**EN.** Step (iv): U computed with representative λ • eImg equals
λ • U computed with representative eImg.
-/
...
/--
**FR.** **Unicité de `U` à phase globale près (version restreinte)** : si l'on
...
**EN.** Uniqueness of U up to a global phase (restricted version):
if V', χ', and U' are reconstructed by replacing, in the formulas of
Defs.lean, the unit representative eImg T := T (e n) with another unit
representative λ • eImg T of the same class (‖λ‖ = 1), then U' and U
differ exactly by the global phase λ. This is a RESTRICTED version of
Bargmann's Theorem 2 in §6: it covers only the freedom actually used by
wigner, the choice of representative of eImg, not a completely arbitrary
U'.
-/
