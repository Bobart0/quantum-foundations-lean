...
private theorem commute_apply {S T : H n →ₗ[ℂ] H n} (h : Commute S T) (x : H n) :
    S (T x) = T (S x) := by
...
/-- Forme pointwise de `Basic.rproj_contract`, appliquée à un vecteur
explicite plutôt qu'au niveau opérateurs — plus directe à combiner avec
`chain_two_0`/`chain_two_1` ci-dessus. -/
private theorem rproj_contract_apply (Λ : LabeledResolution n K) (i i' : Fin K) (x : H n) :
